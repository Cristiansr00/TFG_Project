...
def generateFolderStructure(zip_path:str, k_fold_value:int) -> str:
    ...
    unmarked_dir = raw_dir / "50 Images" / "Unmarked"
    labels_dir = raw_dir / "50 Images" / "Labels"
    ...
    print(f"Imágenes encontradas con etiqueta: {imagenes}")
    ...
    dest_path = paths.DATA_PROCESSED_DIR / f"{TODAY}_{k_fold_value}-Fold_Cross-val"
    dest_path.mkdir(parents=True, exist_ok=True)
    print(f"Configuración K-Fold: {kf}")
...
"""
        ...
        with open(os.path.join(split_dir, 'data.yaml'), 'w', encoding="utf-8") as yaml_file:
